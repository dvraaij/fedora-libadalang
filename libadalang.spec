...
# [Fedora specific] Langkit is not built in the libadalang repository.
Patch:          %{name}-xfail-test-dsl_unparse.patch
...
%global GPRbuild_optflags_soname -largs -Wl,-soname=%{name}.so.%{version} -gargs

# Build the library.
gprbuild %{GPRbuild_optflags} %{GPRbuild_optflags_soname} \
...
gprinstall --prefix=%{buildroot}%{_prefix} --no-manifest --no-build-var \
           %{GPRinstall_flags} -XBUILD_MODE=prod -XLIBRARY_TYPE=relocatable \
           build/libadalang.gpr

# Install the tools.
gprinstall --prefix=%{buildroot}%{_prefix} --no-manifest --no-build-var \
           --create-missing-dirs --mode=usage -XBUILD_MODE=prod -XLIBRARY_TYPE=relocatable \
           build/mains.gpr
...
cat %{buildroot}%{_GNAT_project_dir}/%{name}.gpr
